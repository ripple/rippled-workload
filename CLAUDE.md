...
v1 operator — **Batch inner-txn carrier** (`_carry_unknown_inner_txn_type`): a validly-signed outer Batch (≥2 inners) whose inner carries `TransactionType=60000`. Outer parses + signature verifies, exercising how an inner transaction with an unregistered type is handled downstream of outer deserialization. Restores the map on exit; single-process asyncio + unused sentinel name makes concurrent encodes safe.
