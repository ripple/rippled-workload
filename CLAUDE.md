...
The band above is codec-legal — every blob parses at the top level and reaches doApply, the highest-yield surface per submit. `rawfuzz` is a low-weight escalation (`RAW_CHANCE`) inside `submit_fuzzed` that corrupts **through** the codec's gatekeeping: encodings a conformant client library can never emit. `escalate(name, ops)` returns an `Escalation` (any of `dict_mutate` / `encode_ctx` / `blob_mutate`) when a weighted operator applies to `name`, else `None` (caller falls back to codec-legal). Byte- and field-generic ops apply to **every** transaction type; `drop_end_marker` is gated to `_CONTAINER_TYPES` (Batch, SignerListSet — the only types with an unconditional top-level STArray to strip a terminator from) and `unknown_inner_txn_type` to Batch. Same `workload::fuzz` event namespace; the op is tagged in `ops` (`raw:*`). If a raw op yields a byte-identical blob (nothing to corrupt), `submit_raw` raises `BlobUnchanged` → `fuzz_skipped`, so a no-op never submits the still-valid signed transaction (which would move real funds and mislabel a success as fuzz).
...
- **Surface 1 — structural byte corruption** (post-sign, via `blob_mutate`): corrupt the serialized blob after signing. The signature no longer covers it, but rippled's deserializer runs *before* signature checks, so this targets the parser. Phase A ops are codec-blind: `truncate`, `trailing_garbage`, `byte_flip`. Phase B ops ride the assembler: `duplicate_field`, `reorder_fields` (non-canonical order), `unknown_field_code`, `vl_length_lie` (bogus VL size prefix), `drop_end_marker` (strip an STObject/STArray terminator). Some (duplicate/reorder/drop_end_marker) are lenient-decodable by xrpl-py but non-canonical, so they target rippled's stricter C++ parser rather than the codec.
...
Self-seeding families (no setup step) track state purely from validated results: `w.oracles` (XLS-47 Price Oracle) — the valid `OracleSet` create path seeds it; `OracleDelete`/update return early until oracles exist.
