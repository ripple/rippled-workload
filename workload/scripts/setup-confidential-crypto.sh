...
# — a mismatch would have rippled reject every proof, so divergence fails the build.
set -euo pipefail
...
if [ "$version" != "$rippled_version" ]; then
    echo "FAIL: mpt-crypto divergence — xrpl-py($XRPL_PY_REF)=$version" \
         "rippled($RIPPLED_REF)=$rippled_version. Client proofs would be rejected" \
         "on-ledger; align the two pins before building." >&2
    exit 1
fi
