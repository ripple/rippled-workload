...
- state_updater: optional function(workload, tx_json, meta) called by WS listener on validated tesSUCCESS
...
from workload.transactions.nft import (
    nftoken_mint, nftoken_burn, nftoken_modify,
...
def _extract_created_id(meta, entry_type):
...
def _extract_deleted_id(meta, entry_type):
    for node in meta.get("AffectedNodes", []):
...
def _on_trust_set(w, tx, meta):
    ...
    if isinstance(limit, dict):
        w.trust_lines.append(TrustLine(account_a=tx["Account"], account_b=limit.get("issuer", ""), currency=limit.get("currency", "")))
...
def _on_credential_create(w, tx, meta):
    w.credentials.append(Credential(
        issuer=tx["Account"], subject=tx.get("Subject", ""),
...
def _on_loan_broker_set(w, tx, meta):
    ...
    if broker_id:
        w.loan_brokers.append(LoanBroker(owner=tx["Account"], loan_broker_id=broker_id, vault_id=tx.get("VaultID", "")))
...
def _on_loan_set(w, tx, meta):
    ...
    if loan_id:
        w.loans.append(Loan(borrower=tx["Account"], loan_id=loan_id, loan_broker_id=tx.get("LoanBrokerID", "")))
...
REGISTRY = [
    ("NFTokenMint",              "/nft/mint/random",                    nftoken_mint,                lambda w: (w.accounts, w.nfts, w.client),                              _on_nftoken_mint),
    ("NFTokenBurn",              "/nft/burn/random",                    nftoken_burn,                lambda w: (w.accounts, w.nfts, w.client),                              _on_nftoken_burn),
    ("NFTokenModify",            "/nft/modify/random",                  nftoken_modify,              lambda w: (w.accounts, w.nfts, w.client),                              None),
    ("NFTokenCreateOffer",       "/nft/create_offer/random",            nftoken_create_offer,        lambda w: (w.accounts, w.nfts, w.nft_offers, w.client),                _on_nftoken_create_offer),
    ("NFTokenCancelOffer",       "/nft/cancel_offer/random",            nftoken_cancel_offer,        lambda w: (w.accounts, w.nft_offers, w.client),                        _on_nftoken_cancel_offer),
    ("NFTokenAcceptOffer",       "/nft/accept_offer/random",            nftoken_accept_offer,        lambda w: (w.accounts, w.nfts, w.nft_offers, w.client),                None),
    ("AccountSet",               "/account/set/random",                 account_set_random,          lambda w: (w.accounts, w.client),                                      None),
    ("TrustSet",                 "/trustline/create/random",            trustline_create,            lambda w: (w.accounts, w.trust_lines, w.client),                       _on_trust_set),
    ("Payment",                  "/payment/random",                     payment_random,              lambda w: (w.accounts, w.trust_lines, w.mpt_issuances, w.client),      None),
    ("TicketCreate",             "/tickets/create/random",              ticket_create,               lambda w: (w.accounts, w.client),                                      _on_ticket_create),
    ("TicketUse",                "/tickets/use/random",                 ticket_use,                  lambda w: (w.accounts, w.client),                                      None),
    ("Batch",                    "/batch/random",                       batch_random,                lambda w: (w.accounts, w.client),                                      None),
    ("MPTokenIssuanceCreate",    "/mpt/create/random",                  mpt_create,                  lambda w: (w.accounts, w.mpt_issuances, w.client),                     _on_mpt_create),
    ("MPTokenAuthorize",         "/mpt/authorize/random",               mpt_authorize,               lambda w: (w.accounts, w.mpt_issuances, w.client),                     None),
    ("MPTokenIssuanceSet",       "/mpt/set/random",                     mpt_issuance_set,            lambda w: (w.accounts, w.mpt_issuances, w.client),                     None),
    ("MPTokenIssuanceDestroy",   "/mpt/destroy/random",                 mpt_destroy,                 lambda w: (w.accounts, w.mpt_issuances, w.client),                     _on_mpt_destroy),
    ("CredentialCreate",         "/credential/create/random",           credential_create,           lambda w: (w.accounts, w.credentials, w.client),                       _on_credential_create),
    ("CredentialAccept",         "/credential/accept/random",           credential_accept,           lambda w: (w.accounts, w.credentials, w.client),                       None),
    ("CredentialDelete",         "/credential/delete/random",           credential_delete,           lambda w: (w.accounts, w.credentials, w.client),                       _on_credential_delete),
    ("VaultCreate",              "/vault/create/random",                vault_create,                lambda w: (w.accounts, w.vaults, w.trust_lines, w.mpt_issuances, w.client), _on_vault_create),
    ("VaultDeposit",             "/vault/deposit/random",               vault_deposit,               lambda w: (w.accounts, w.vaults, w.client),                            None),
    ("VaultWithdraw",            "/vault/withdraw/random",              vault_withdraw,              lambda w: (w.accounts, w.vaults, w.client),                            None),
    ("VaultSet",                 "/vault/set/random",                   vault_set,                   lambda w: (w.accounts, w.vaults, w.client),                            None),
    ("VaultDelete",              "/vault/delete/random",                vault_delete,                lambda w: (w.accounts, w.vaults, w.client),                            _on_vault_delete),
    ("VaultClawback",            "/vault/clawback/random",              vault_clawback,              lambda w: (w.accounts, w.vaults, w.client),                            None),
    ("PermissionedDomainSet",    "/domain/set/random",                  permissioned_domain_set,     lambda w: (w.accounts, w.domains, w.client),                           _on_domain_set),
    ("PermissionedDomainDelete", "/domain/delete/random",               permissioned_domain_delete,  lambda w: (w.accounts, w.domains, w.client),                           _on_domain_delete),
    ("DelegateSet",              "/delegate/set/random",                delegate_set,                lambda w: (w.accounts, w.client),                                      None),
    ("LoanBrokerSet",            "/loan/broker/set/random",             loan_broker_set,             lambda w: (w.accounts, w.vaults, w.loan_brokers, w.client),            _on_loan_broker_set),
    ("LoanBrokerDelete",         "/loan/broker/delete/random",          loan_broker_delete,          lambda w: (w.accounts, w.loan_brokers, w.client),                      _on_loan_broker_delete),
    ("LoanBrokerCoverDeposit",   "/loan/broker/cover/deposit/random",   loan_broker_cover_deposit,   lambda w: (w.accounts, w.loan_brokers, w.client),                      None),
    ("LoanBrokerCoverWithdraw",  "/loan/broker/cover/withdraw/random",  loan_broker_cover_withdraw,  lambda w: (w.accounts, w.loan_brokers, w.client),                      None),
    ("LoanSet",                  "/loan/set/random",                    loan_set,                    lambda w: (w.accounts, w.loan_brokers, w.loans, w.client),             _on_loan_set),
    ("LoanDelete",               "/loan/delete/random",                 loan_delete,                 lambda w: (w.accounts, w.loans, w.client),                             _on_loan_delete),
    ("LoanManage",               "/loan/manage/random",                 loan_manage,                 lambda w: (w.accounts, w.loan_brokers, w.loans, w.client),             None),
    ("LoanPay",                  "/loan/pay/random",                    loan_pay,                    lambda w: (w.accounts, w.loans, w.client),                             None),
]
